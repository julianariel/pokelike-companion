...
export function getEffectiveness(attackType: PokemonType, defendType: PokemonType): number {
  return TYPE_CHART[attackType][defendType] ?? 1;
}

export function summarizeAttackType(attackType: PokemonType) {
  ...
  for (const defendType of POKEMON_TYPES) {
    ...
    if (multiplier >= 2) superEffective.push(defendType);
    else if (multiplier === 0) immune.push(defendType);
    else if (multiplier < 1) resisted.push(defendType);
  }
